...
// MapLibre parses vector tiles in a worker it cannot locate on its own: inside a
// bundle `import.meta.url` is *our* chunk, so `getWorkerUrl()` returns a
// plausible but non-existent `/_next/static/chunks/maplibre-gl-worker.mjs` — and
// because that string is non-empty, the upstream unpkg fallback never runs. The
